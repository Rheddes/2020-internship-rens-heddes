...
def determine_total_risk(measured_profile) -> int:
...
class VulnerabilityRiskEngine:
    ...
    def calculate_risk(self, vertex_set: set, graph: AttributesGraph) -> int:
        """
        ...
        for callable_id in vertex_set:
            ...
            if vulnerabilities:
                highest_score = max([float(vulnerability['scoreCVSS3']) for vulnerability in vulnerabilities.values()])
                if self.thresholds['low']['low'] < highest_score <= self.thresholds['low']['high']:
                    risk_profile.add_callable(callable_id, RiskLevel.LOW)
                elif self.thresholds['moderate']['low'] < highest_score <= self.thresholds['moderate']['high']:
                    risk_profile.add_callable(callable_id, RiskLevel.MODERATE)
                elif self.thresholds['high']['low'] < highest_score <= self.thresholds['high']['high']:
                    risk_profile.add_callable(callable_id, RiskLevel.HIGH)
                elif self.thresholds['very high']['low'] < highest_score <= self.thresholds['very high']['high']:
                    risk_profile.add_callable(callable_id, RiskLevel.VERY_HIGH)
            else:
                risk_profile.add_callable(callable_id, RiskLevel.NONE)
        print(risk_profile)
        return determine_total_risk(risk_profile)
        ...
        for callable in graph.vertices:
            ...
            if vulnerabilities:
                severenessList.extend(
                    [float(vulnerability['scoreCVSS3']) for vulnerability in vulnerabilities.values()])
